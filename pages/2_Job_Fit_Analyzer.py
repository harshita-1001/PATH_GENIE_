...
match_model = tf.keras.models.load_model("Neural_Networks/match_score_model.h5", compile=False)
match_model.compile(optimizer='adam', loss='mean_squared_error', metrics=['mae'])
match_vectorizer = joblib.load("Neural_Networks/match_vectorizer.pkl")

# Load skill vocabulary and job roles
@st.cache_data
def load_skill_list():
    with open("datasets/unique_skills.txt", "r") as f:
